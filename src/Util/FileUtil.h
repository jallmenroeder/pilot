...
namespace FileUtil {
    ...
    static std::vector<uint32_t> readFileBinary(std::string_view path) {
        ...
        file.seekg(0);
        file.read(reinterpret_cast<char *>(buffer.data()), long(sizeof(uint32_t) * buffer.size()));
        file.close();
    ...
    static void writeFile(std::string_view path, const std::vector<uint32_t> &data) {
        ...
        file.write(reinterpret_cast<const char *>(data.data()), long(data.size() * sizeof(uint32_t)));
        file.close();
    }
